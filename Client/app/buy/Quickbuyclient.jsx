...
const TrackOrder = ({ darkMode }) => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API}/api/guest/track`, {
        method: 'POST',
  ...
  const getNetworkColor = (network) => {
    switch (network) {
      case 'YELLO': return 'bg-yellow-500';
      case 'TELECEL': return 'bg-red-500';
      case 'AT_PREMIUM': return 'bg-purple-500';
      ...
      {isOpen && (
        <div className={`mt-3 p-4 rounded-xl border shadow-lg ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
          <form onSubmit={handleSearch} className="space-y-4">
            <div className={`flex gap-2 p-1 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2 px-4 rounded-md text-sm font-medium transition-all ${
                  searchType === 'phone'
                    ? darkMode ? 'bg-gray-600 text-indigo-400 shadow' : 'bg-white text-indigo-600 shadow'
            ...
            >
              {loading ? (<><Loader2 className="w-5 h-5 animate-spin" /> Searching...</>) : (<><Search className="w-5 h-5" /> Track Order</>)}
            </button>
          </form>
            ...
            <div className="mt-4 space-y-3">
              ...
              {orders.map((order, index) => (
                <div key={index} className={`p-4 rounded-lg border ${darkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full ${getNetworkColor(order.network)} flex items-center justify-center text-white font-bold text-xs`}>
                    ...
                    <div className={`px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(order.orderStatus)} flex items-center gap-1`}>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      ...
                      <p className={`font-mono text-xs break-all ${darkMode ? 'text-white' : 'text-gray-900'}`}>{order.reference}</p>
                    </div>
                    <div>
                      <p className={darkMode ? 'text-gray-400' : 'text-gray-500'}>Amount</p>
                      <p className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>GH₵{order.price?.toFixed(2)}</p>
                    </div>
                    <div>
                      <p className={darkMode ? 'text-gray-400' : 'text-gray-500'}>Payment</p>
                      <p className={darkMode ? 'text-white' : 'text-gray-900'}>{order.paymentStatus}</p>
                    </div>
                    <div>
                      <p className={darkMode ? 'text-gray-400' : 'text-gray-500'}>Date</p>
                      <p className={darkMode ? 'text-white' : 'text-gray-900'}>{new Date(order.createdAt).toLocaleDateString()}</p>
                    </div>
                  </div>
                  ...
                  {order.trackingId && (
                    <div className={`mt-3 p-3 rounded-lg border ${darkMode ? 'bg-indigo-900/20 border-indigo-800' : 'bg-indigo-50 border-indigo-200'}`}>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <p className={`font-medium text-sm ${darkMode ? 'text-indigo-300' : 'text-indigo-900'}`}>Tracking: {order.trackingId}</p>
                      </div>
                      {order.deliveryInfo && <p className={`text-xs ${darkMode ? 'text-indigo-400' : 'text-indigo-700'}`}>{order.deliveryInfo}</p>}
                    </div>
                  )}
                </div>
              ))}
              ...
              <div className={`mt-4 p-4 rounded-lg border ${darkMode ? 'bg-gradient-to-r from-green-900/20 to-emerald-900/20 border-green-800' : 'bg-gradient-to-r from-green-50 to-emerald-50 border-green-200'}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className={`font-semibold mb-1 ${darkMode ? 'text-green-300' : 'text-green-900'}`}>Haven&apos;t received your bundle?</p>
                    <p className={`text-sm mb-3 ${darkMode ? 'text-green-400' : 'text-green-700'}`}>Join our WhatsApp channel for support.</p>
      ...
      <div className="bg-gray-900 rounded-2xl max-w-sm w-full overflow-hidden shadow-2xl animate-modal-pop">
        ...
        <div className="p-6 text-center">
          ...
          <div className="bg-indigo-900/30 border border-indigo-600/50 rounded-xl p-4 mb-4">
            ...
            <p className="text-indigo-200 text-sm">Usually <strong>10 mins - 24 hours</strong></p>
          </div>

          <p className="text-amber-400 text-sm mb-6">Data cannot be reversed once sent!</p>
...
export default function QuickBuyClient() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      const response = await fetch(`${API}/api/guest/products`);
      const data = await response.json();
  ...
  const handleShowConfirmation = (product, index) => {
    ...
    if (!validatePhoneNumber(phoneNumber, product.network)) {
      setBundleMessages(prev => ({ ...prev, [index]: { text: `Please enter a valid ${product.network === 'YELLO' ? 'MTN' : product.network === 'TELECEL' ? 'Telecel' : 'AirtelTigo'} number`, type: 'error' } }));
      return;
    }
    if (!customerName) {
  ...
  const handleConfirmedPurchase = async () => {
    ...
    try {
      const response = await fetch(`${API}/api/guest/initialize`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (data.status === 'success' && data.data.authorizationUrl) {
        showToast(`${product.capacity}GB bundle purchase initiated!`, 'success');
  ...
  const filteredProducts = products.filter(p => p.network === selectedNetwork).sort((a, b) => a.capacity - b.capacity);
      ...
      <div className={`sticky top-0 z-40 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border-b`}>
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/SignIn" className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${darkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-700 hover:bg-gray-100'}`}>
              Sign In
            </Link>
            <Link href="/SignUp" className="px-4 py-2 text-sm font-semibold bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg transition-colors">
              Sign Up
            </Link>
          </div>
        </div>
        ...
        <div className="mb-8">
          ...
          <div className="flex gap-4 flex-wrap">
            {availableNetworks.map((network) => (
              <button
                key={network}
                onClick={() => { setSelectedNetwork(network); setSelectedProductIndex(null); }}
